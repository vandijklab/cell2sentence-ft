...
class CustomTrainingArguments:
    model_name: str = field(
        default='gpt2',
        metadata={"help": "Hugging Face model name."}
    )
    seed: int = field(
        default=42,
        metadata={"help": "Seed for shuffling training dataset."}
    )
    ...
    set_torch_seed_manually: bool = field(
        default=False,
        metadata={"help": "Seed for PyTorch."}
    )
    torch_cuda_seed: int = field(
        default=42,
        metadata={"help": "Seed for PyTorch CUDA."}
    )
    ...
    eval_steps: int = field(
        default=100,
        metadata={"help": "If evaluation_strategy is set to 'steps', will evaluate every number of steps here."}
    )
    ...
    save_strategy: str = field(
        default="steps",
        metadata={"help": "Whether to save model checkpoints on steps, epochs, or none."}
    )
    save_steps: int = field(
        default=500,
        metadata={"help": "If save_strategy is set to 'steps', will save model checkpoint every number of steps here."}
    )
    save_total_limit: int = field(
        default=100,
        metadata={"help": "Maximum number of model checkpoints saved in output directory."
        " Will overwrite earlier checkpoints if limit is exceeded."}
    )
    per_device_train_batch_size: int = field(
        default=16,
        metadata={"help": "Per device batch size used during training."}
    )
    per_device_eval_batch_size: int = field(
        default=16,
        metadata={"help": "Per device batch size used during evaluation."}
    )
    ...
    wandb_project_name: str = field(
        default="cell2sentence",
        metadata={"help": "Wandb project name to save to."}
    )
    ...
    torchdynamo: Optional[str] = field(
        default=None,
        metadata={"help": "Backend compiler for torch dynamo."}
    )
    torch_compile_backend: Optional[str] = field(
        default=None,
        metadata={"help": "Backend compiler for torch compile."}
    )
    dynamo_cache_size_limit: int = field(
        default=64,
        metadata={"help": "Number of graphs to cache for torch compile."}    
    )
    dynamo_verbose: bool = field(
        default=False,
        metadata={"help": "Make dynamo config set to verbose."}    
    )
    fp16: bool = field(
        default=False,
    ...
    ddp_backend: str = field(
        default="nccl",
        metadata={"help": "Backend for distributed data parallelism."}  
    )
    dataloader_num_workers: int = field(
        default=0,
        metadata={"help": "Number of workers to use for dataloader."}
    )
    gradient_checkpointing: bool = field(
        default=False,
        metadata={"help": "Whether to checkpoint gradients during training. Improves GPU memory consumption."}
    )
    gradient_accumulation_steps: int = field(
        default=1,
        metadata={"help": "Number of forward passes before backpropagation during training. Per device."}
    )
...
def main():
    ...
    def preprocess_function(examples):
        ...
        for i in range(batch_size):
            ...
            if prompt_type == 0:
                ...
                input = construct_prediction_template(" ".join(examples['input_ids'][i].split(" ")[:100]))
    ...
    def data_collator(examples):
        ...
        for i in range(len(examples)):
            ...
            final_input_ids = [tokenizer.pad_token_id]*(max_length - len(sample_input_ids)) + sample_input_ids
            final_attention_mask = [0] * (max_length - len(sample_input_ids)) + attention_mask
            final_label_input_ids = [-100] * (max_length - len(sample_input_ids)) + label_input_ids
