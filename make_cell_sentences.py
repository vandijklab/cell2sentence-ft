...
def create_partition_arrow_ds(filename, data_split="train"):
    ...
    preprocessed_adata = anndata.read_h5ad(
        os.path.join(CURRENT_DIR, "preprocessed_adata.h5ad")
    )
    dataset_split_sample_indices = np.load(
        os.path.join(CELL_SENTENCES_DIR, f"{data_split}_partition_indices.npy")
    )
    dataset_split_preprocessed_adata = preprocessed_adata[
        dataset_split_sample_indices, :
    ].copy()
...
def main():
    create_partition_arrow_ds(
        filename=os.path.join(CELL_SENTENCES_DIR, "train_human.txt"), data_split="train"
    )
    create_partition_arrow_ds(
        filename=os.path.join(CELL_SENTENCES_DIR, "valid_human.txt"), data_split="val"
    )
    create_partition_arrow_ds(
        filename=os.path.join(CELL_SENTENCES_DIR, "test_human.txt"), data_split="test"
    )
